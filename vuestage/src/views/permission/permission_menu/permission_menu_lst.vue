<template>
  <div class="app-container permission_menu_lst">
    ...
    <el-row type="flex" class="row-bg" justify="center">
      <el-col>
        <el-button type="primary" icon="el-icon-plus" @click.native="$router.push('/permission/permission_menu/permission_menu_add')">添加</el-button>
      </el-col>
    </el-row>
    <el-table
      :data="List"
      ...
      >
        <template slot-scope="scope">
          ...
          <span v-if="column.dataIndex === 'hidden'&&scope.row[column.dataIndex]===1">显示</span>
          <span v-else-if="column.dataIndex === 'hidden'&&scope.row[column.dataIndex]===2">隐藏</span>
...
<script>
import { permissionMenuDelete, permissionMenuLst } from '@/api/permission/permission_menu'
...
export default {
  ...
  methods: {
    ...
    handleDelete(index, row) {
      permissionMenuDelete({ id: row.id }).then(res => {
