...
<script>
import { permissionSrcAdd, permissionSrcOptions, permissionControllerOptions, permissionActionOptions } from '@/api/permission/permission_src'
export default {
  ...
  methods: {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          permissionSrcAdd(this.ruleForm).then(res => {
            this.$message({
              message: '添加成功',
              type: 'success',
              onClose: function() {
